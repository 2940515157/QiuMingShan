...
function initaccordion(obj,data) {
    ...
    for(var i = 0;i<data.length;i++){
        ...
        //设置外层菜单的显示图标
        clonepanelobj.find("div[class='panel-heading'] a").attr("class",data[i].iconUrl);
        ...
        for(var j=0;j<data[i].childMenu.length;j++){
            ...
            //设置子菜单的显示图标
            cloneaobj.addClass(data[i].childMenu[j].iconUrl);
            cloneaobj.attr("iconUrl",data[i].childMenu[j].iconUrl);
